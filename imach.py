...
def is_valid_time(start_dt, title):
    ...
    if 5.5 <= w <= 17.5:
        whitelist = [
            'badminton', 'bwf', 'thomas', 'uber', 'sudirman', 'orleans', 'master', 'masters', 'open', 'tour', 'super', 'all england',
...
def is_match_akurat_v3(epg_name, epg_id, m3u_name):
    ...
    if not any(k in e_k for k in ['badminton', 'orleans', 'master', 'bwf']) and en != mn and 'spotv' not in e_k: return False
...
def main():
    ...
    for url in M3U_URLS:
        ...
        is_event_file = "imach" in url.lower()
                    ...
                    if is_event_file:
                        ...
                        if ev_m:
                            ...
                            if ev_stop > now_wib:
                                ...
                                if is_live:
                                    judul = f"{get_flag(ev_title)} 🛡️{jam_tayang} - {ev_title}"
                                    inf = f'{clean_attrs} group-title="SIARAN LANGSUNG" tvg-id="" tvg-logo="{orig_logo}", {judul}'
                            ...
                            judul = f"{get_flag(m3u_name)} 🛡️ EVENT - {m3u_name}"
                            inf = f'{clean_attrs} group-title="SIARAN LANGSUNG" tvg-id="" tvg-logo="{orig_logo}", {judul}'
                            keranjang_match[key]["links"].append({"prio": 0, "orig_url": stream_url, "data": [inf] + extra_tags + [stream_url]})
                            ...
                            if is_match_akurat_v3(ename, cid, m3u_name) and cid in match_data:
                                for ev in match_data[cid]:
                                    ...
                                    if ev["live"]:
                                        ...
                                        judul = f"{get_flag(m3u_name)} 🛡️ {jam_tayang} WIB - {ev['title']} [{m_disp}]"
                                        inf = f'{clean_attrs} group-title="SIARAN LANGSUNG" tvg-id="{cid}" tvg-logo="{final_logo}", {judul}'
